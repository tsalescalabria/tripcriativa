...
?>
    ...
    <div class="wpte-field wpte-floated">
        <label class="wpte-field-label"><?php _e( 'Duration', 'wp-travel-engine' ); ?></label>
        <div class="wpte-multi-fields wpte-floated">
            <input type="number" min="1" step="1" name="wp_travel_engine_setting[trip_duration]" value="<?php echo $trip_duration ? esc_attr( $trip_duration ) : '' ?>" placeholder="<?php _e( 'Enter Duration', 'wp-travel-engine' ); ?>">
    ...
    <div class="wpte-field wpte-onoff-block">
        <a href="Javascript:void(0);" class="wpte-onoff-toggle <?php echo $enable_cut_off ? 'active' : ''; ?>">
            <label for="wpte-enable-cut-off" class="wpte-field-label"><?php _e( 'Enable Cut-Off Time', 'wp-travel-engine' ); ?><span class="wpte-onoff-btn"></span></label>
        </a>
        ...
        <span class="wpte-tooltip"><?php _e( 'Enable the cut-off time for the trip bookings. The cut-off time will be the time before which bookings are allowed for the trip.', 'wp-travel-engine' ) ?></span>
        <div <?php echo $enable_cut_off ? 'style="display:block;"' : ''; ?> class="wpte-onoff-popup">
            <div class="wpte-field wpte-multi-fields wpte-floated">
                <label class="wpte-field-label"><?php _e( 'Cut-Off Time', 'wp-travel-engine' ); ?></label>
                <div class="wpte-floated">
                    <input type="number" step="1" min="1" name="wp_travel_engine_setting[trip_cut_off_time]" value="<?php echo $trip_cut_off_time ? $trip_cut_off_time : ''; ?>" placeholder="<?php _e( 'Enter cut-off time', 'wp-travel-engine' ); ?>">
                ...
                <span class="wpte-tooltip"><?php _e( 'Enter trip cut-off value in number of days. If you set your cutoff time to 1 day, the product cannot be booked with a start date today. If 2 days, the product cannot be booked with a start date today and tomorrow etc.', 'wp-travel-engine' ) ?></span>
            </div>
        </div>
    </div>
    ...
    <div class="wpte-field wpte-onoff-block">
        <a href="Javascript:void(0);" class="wpte-onoff-toggle <?php echo $min_max_age_enable ? 'active' : ''; ?>">
            <label for="wpte-enable-min-max-age" class="wpte-field-label"><?php _e( 'Set Minimum And Maximum Age', 'wp-travel-engine' ); ?><span class="wpte-onoff-btn"></span></label>
        </a>
        ...
        <span class="wpte-tooltip"><?php _e( 'Enable minimum and maximum age required restriction for booking this trip.', 'wp-travel-engine' ) ?></span>
        <div <?php echo $min_max_age_enable ? 'style="display:block;"' : ''; ?> class="wpte-onoff-popup">
            <div class="wpte-field wpte-minmax wpte-floated">
                <div class="wpte-min">
                    <label class="wpte-field-label"><?php _e( 'Minimum Age', 'wp-travel-engine' ); ?></label>
                    <input type="number" step="1" min="1" name="wp_travel_engine_trip_min_age" value="<?php echo esc_attr( $trip_minimum_age ); ?>" placeholder="<?php _e( 'Enter minimum age', 'wp-travel-engine' ) ?>">
                </div>
                <div class="wpte-max">
                    <label class="wpte-field-label"><?php _e( 'Maximum Age', 'wp-travel-engine' ); ?></label>
                    <input type="number" step="1" min="1" name="wp_travel_engine_trip_max_age" value="<?php echo esc_attr( $trip_maximum_age ); ?>" placeholder="<?php _e( 'Enter maximum age', 'wp-travel-engine' ) ?>">
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="wpte-field wpte-onoff-block">
        <a href="Javascript:void(0);" class="wpte-onoff-toggle <?php echo $minmax_pax_enable ? 'active' : ''; ?>">
            <label for="wpte-fsd-enable-min-max" class="wpte-field-label"><?php _e( 'Set Minimum And Maximum Participants', 'wp-travel-engine' ); ?><span class="wpte-sublabel"><?php _e( '(Optional)', 'wp-travel-engine' ); ?></span><span class="wpte-onoff-btn"></span></label>
        </a>
        ...
        <span class="wpte-tooltip"><?php _e( 'Enable minimum and maximum participants for booking this trip.', 'wp-travel-engine' ); ?></span>
        <div <?php echo $minmax_pax_enable ? 'style="display:block;"' : ''; ?> class="wpte-onoff-popup">
            <div class="wpte-field wpte-minmax wpte-floated">
                <div class="wpte-min">
                    <label class="wpte-field-label"><?php _e( 'Minimum Participants', 'wp-travel-engine' ); ?></label>
                ...
                <div class="wpte-max">
                    <label class="wpte-field-label"><?php _e( 'Maximum Participants', 'wp-travel-engine' ); ?></label>
    ...
    <?php if ( $next_tab ) : ?>
        <div class="wpte-field wpte-submit">
            <input data-tab="general" data-post-id="<?php echo esc_attr( $post->ID ); ?>" data-nonce="<?php echo esc_attr( wp_create_nonce( 'wpte-trip-tab-save-continue' ) ); ?>" data-next-tab="<?php echo esc_attr( $next_tab['callback_function'] ); ?>" class="wpte_save_continue_link" type="submit" name="wpte_trip_tabs_save_continue" value="<?php _e( 'Save &amp; Continue', 'wp-travel-engine' ); ?>">
        </div>
    <?php endif;
